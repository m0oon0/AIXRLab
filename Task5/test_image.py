...
def draw_on_image(args, image_id, laeo_list, image_path, out_path):
    # img_name = image_id
    # assert args.dataset_file in ['laeo'], args.dataset_file
    ...
    img_result = cv2.imread(image_path, cv2.IMREAD_COLOR)
    bbox_list = []
    # color = random_color()
    color = (0, 255, 0)
...
def test_image():
    ...
    img_id_list = [0]
    org_sizes = [[img.shape[2], img.shape[3]]]
    action_pred_logits = outputs['action_pred_logits']
    person2_pred_logits = outputs['person2_pred_logits']
    person2_pred_boxes = outputs['person2_pred_boxes']
    person1_pred_logits = outputs['person1_pred_logits']
    ...
    for idx_img in range(len(action_pred_logits)):
        ...
        outpath = './output/{}_output.jpg'.format(os.path.basename(img_path).split('.')[0])
        draw_on_image(args, item['image_id'], item['laeo_list'], image_path=img_path, out_path=outpath)
